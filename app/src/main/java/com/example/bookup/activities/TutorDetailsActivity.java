...
public class TutorDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Get Tutor object from Intent or by ID
        String tutorIdFromIntent = getIntent().getStringExtra("tutorId");
        
        if (tutorIdFromIntent != null) {
            // Loading tutor by ID from chat profile click
            loadTutorById(tutorIdFromIntent);
        } else if (getIntent().hasExtra(EXTRA_TUTOR)) {
        ...
        btnBookSession.setOnClickListener(v -> {
            if (mAuth.getCurrentUser() != null) {
                if (mAuth.getCurrentUser().getUid().equals(currentTutor.getUid())) {
                    ...
                    // Navigate to BookingSessionActivity
                    Intent bookingIntent = new Intent(TutorDetailsActivity.this, BookingSessionActivity.class);
    ...
    private void loadTutorById(String tutorId) {
        // Load tutor from Firestore by ID (when clicked from chat)
        setLoading(true);
        // TODO: Implement Firestore query to fetch tutor by ID
        // For now, create a dummy Tutor object
        currentTutor = new Tutor();
        currentTutor.setUid(tutorId);
        displayTutorDetails();
        setLoading(false);
    }
